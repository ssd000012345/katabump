...
async function fetchRawProxies() {
    const sources = [
        ...
        { name: 'geonode', url: 'https://proxylist.geonode.com/api/proxy-list?limit=20&page=1&sort_by=lastChecked&sort_type=desc&protocols=http' }
    ];
    const all = [];
    for (const src of sources) {
        try {
            ...
            for (const l of lines) {
                const c = l.trim().replace(/\r/g, '');
                const url = c.startsWith('http') ? c : `http://${c}`;
                try { const u = new URL(url); if (u.hostname && u.port) all.push(url); } catch (e) {}
            }
            console.log(`   [代理池] ${src.name}: ${lines.length} 个`);
...
async function testSingleProxy(proxyUrl) {
    ...
    try {
        ...
        return { url: proxyUrl, ok: true, ms: Date.now() - start };
    } catch (e) {
        return { url: proxyUrl, ok: false, ms: 99999 };
    }
}

async function batchTestProxies(rawProxies) {
    const concurrency = 15;
    const results = [];
    for (let i = 0; i < rawProxies.length; i += concurrency) {
        const batch = rawProxies.slice(i, i + concurrency);
        const batchResults = await Promise.all(batch.map(p => testSingleProxy(p)));
        results.push(...batchResults);
        const done = Math.min(i + concurrency, rawProxies.length);
        const ok = results.filter(r => r.ok).length;
        process.stdout.write(`\r   测速: ${done}/${rawProxies.length}  可用: ${ok}`);
    }
    console.log('');

    const good = results.filter(r => r.ok).sort((a, b) => a.ms - b.ms);
    console.log(`   最快: ${good.slice(0, 5).map(p => `${p.url} (${p.ms}ms)`).join(', ')}`);
    return good.map(p => p.url);
}
// =====================================================

async function switchToProxy(oldBrowser) {
    while (fastProxies.length > 0) {
        const proxyUrl = fastProxies.shift();
        console.log(`   [代理切换] → ${proxyUrl}`);
        const u = new URL(proxyUrl);
        ...
        catch (e) { console.log('   重连失败，下一个'); continue; }
        ...
        console.log(`   [代理切换] ✅ 就绪: ${currentProxy.server}`);
        return { page, browser, ok: true };
    }
    console.log('   [代理切换] 5 个都用完了');
    return { ok: false };
}
...
(async () => {
    ...
    if (!currentProxy) {
        ...
        console.log(`共获取 ${raw.length} 个代理，开始并行测速...`);
        fastProxies = await batchTestProxies(raw);
        console.log(`可用: ${fastProxies.length} 个\n`);
    }
    ...
    if (needProxyRetry.length > 0 && !currentProxy) {
        console.log(`\n===== ${needProxyRetry.length} 个用户需代理 =====`);
        const sw = await switchToProxy(browser);
        if (!sw.ok) { console.log('代理切换失败'); await sendTelegramMessage('❌ *代理切换失败*'); }
        else {
